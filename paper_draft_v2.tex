...
pipeline. Using only publicly available data from the NeurIPS 2025 Open Polymer Challenge
dataset (3,502 polymers), our multi-task GNN simultaneously predicts density
($R^2 = 0.757$, MAE $= 0.0294$ g/ml) and normalized crystallization temperature
($R^2 = 0.512$, MAE $= 0.0364$ in normalized scale, MAE $= 0.0028$ in original unit) from SMILES representations.
Multi-task learning improves density prediction by 27\% over single-task GNN baselines
($R^2$: $0.595 \rightarrow 0.757$), supported by a statistically significant negative
...
\begin{enumerate}
    \item A multi-task GNN that simultaneously predicts density and crystallization
    temperature from polymer SMILES, leveraging their physical correlation.
...
labeled polymer data (1,526 density samples in our dataset), we chose a
...
\begin{itemize}
    \item Multi-task GNN achieving $R^2=0.757$ for polymer density using
    only open, publicly available data
...
We use the NeurIPS 2025 Open Polymer Challenge dataset \citep{neurips_opc_2025}
containing 3,502 polymer structures with up to five measured properties.
Table~\ref{tab:dataset} summarizes the data distribution.

\begin{table}[h]
\centering
\caption{Dataset statistics from NeurIPS 2025 Open Polymer Challenge}
\label{tab:dataset}
\begin{tabular}{lcccc}
\toprule
Property & Samples & Min & Max & Mean \\
\midrule
Density (g/ml) & 1,526 & 0.135 & 1.733 & 1.082 \\
Tc (normalized) & 1,404 & 0.025 & 0.954 & 0.247 \\
Both & 1,392 & \multicolumn{3}{c}{39.7\% of total dataset} \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Note on Tc values:} The Tc values in the NeurIPS 2025 dataset
are provided in a normalized scale $[0, 1]$ rather than absolute temperature
units (K or °C). All reported Tc metrics are in this normalized scale.
...
task-specific prediction heads for density and Tc:

\begin{equation}
\hat{y}_{\text{density}}, \hat{y}_{T_c} = f_{\text{density}}(\mathbf{h}),\; f_{T_c}(\mathbf{h})
\end{equation}

\noindent\textbf{Training.}
Both target properties are z-score normalized before training.
...
where $M_d$ and $M_t$ are the index sets of samples with observed
density and Tc, respectively.
...
\caption{Dataset analysis. (Left) Density distribution (n=1,526, mean=1.082 g/ml). (Center) Tc distribution in normalized scale (n=1,404). (Right) Density-Tc correlation scatter plot ($r=-0.246$, $p=1.07\times10^{-20}$, n=1,392).}
\label{fig:data}
\end{figure}
\section{Results}
...
between density and Tc across 1,392 polymers with both measurements.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{paper_figures/fig2_prediction_results.png}
\caption{Multi-task GNN prediction results on test set. 
(Left) Density prediction ($R^2=0.757$, MAE$=0.0294$ g/ml). 
(Right) Tc prediction ($R^2=0.512$, MAE$=0.0364$ normalized).}
...
\begin{tabular}{lcccc}
\toprule
Model & Density $R^2$ & Density MAE (g/ml) & Tc $R^2$ & Tc MAE (norm.) \\
\midrule
Random Forest (9 feat.) & 0.388 & 0.0622 & -- & -- \\
XGBoost (217 feat.) & 0.595 & 0.0402 & -- & -- \\
Single-task GNN & 0.595 & 0.0364 & -- & -- \\
\textbf{Multi-task GNN (ours)} & \textbf{0.757} & \textbf{0.0294} & \textbf{0.512} & \textbf{0.0364} \\
\bottomrule
\end{tabular}
\end{table}

Multi-task learning provides a 27\% improvement in $R^2$ over single-task GNN
($0.595 \rightarrow 0.757$), demonstrating that the negative Density-Tc correlation
($r = -0.246$) provides meaningful complementary signals during joint training.
The Tc MAE of 0.0364 in normalized scale corresponds to 0.0028 in
the original normalized units ($\sigma_{Tc} = 0.0780$).
...
and inverse design using only publicly available data.
Our multi-task GNN achieves $R^2 = 0.757$ for density prediction,
outperforming single-task baselines by 27\%, supported by the
statistically significant negative Density-Tc correlation.
...
The model is limited by training data size (1,526 density samples).
...
and (3) explore diffusion model-based molecular generation as a
complement to the genetic algorithm approach for larger-data regimes,
and (4) incorporate a rule-based polymerizability filter to ensure
...
\url{https://github.com/00mahoon/polyinverse} and
\url{https://huggingface.co/spaces/00mahoon/polyinverse}.
