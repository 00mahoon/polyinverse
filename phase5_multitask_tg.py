...
for epoch in range(500):
    ...
    if (epoch + 1) % 100 == 0:
        model.eval()
        results = {'Density': {'true': [], 'pred': []},
                   'Tc': {'true': [], 'pred': []},
                   'Tg': {'true': [], 'pred': []}}
        with torch.no_grad():
            for batch in test_loader:
                pd_out, pt_out, ptg_out = model(batch)
                for key, pred, y_attr in [
                    ('Density', pd_out, batch.y_density),
                    ('Tc', pt_out, batch.y_tc),
                    ('Tg', ptg_out, batch.y_tg)
                ]:
                    mask = ~torch.isnan(y_attr.squeeze())
                    if mask.sum() > 0:
                        true_vals = y_attr.squeeze()[mask].numpy() * stats[key]['std'] + stats[key]['mean']
                        pred_vals = pred[mask].numpy() * stats[key]['std'] + stats[key]['mean']
                        results[key]['true'].extend(true_vals.tolist())
                        results[key]['pred'].extend(pred_vals.tolist())

        r2s = []
        print(f"\nEpoch {epoch+1}/500 | Loss: {avg_loss:.4f}")
        for key in ['Density', 'Tc', 'Tg']:
            if len(results[key]['true']) > 1:
                r2 = r2_score(results[key]['true'], results[key]['pred'])
                mae = mean_absolute_error(results[key]['true'], results[key]['pred'])
                print(f"  {key:<10} R²: {r2:.3f} | MAE: {mae:.4f}")
                r2s.append(r2)

        avg_r2 = np.mean(r2s)
        if avg_r2 > best_r2:
            best_r2 = avg_r2
            torch.save(model.state_dict(), 'best_multitask_tg.pt')

model.load_state_dict(torch.load('best_multitask_tg.pt'))
model.eval()
results = {'Density': {'true': [], 'pred': []},
           'Tc': {'true': [], 'pred': []},
           'Tg': {'true': [], 'pred': []}}
...
for key in ['Density', 'Tc', 'Tg']:
    r2 = r2_score(results[key]['true'], results[key]['pred'])
    mae = mean_absolute_error(results[key]['true'], results[key]['pred'])
    unit = 'g/ml' if key == 'Density' else '°C' if key == 'Tg' else 'norm'
    print(f"{key:<10} R²: {r2:.3f} | MAE: {mae:.4f} {unit}")

fig, axes = plt.subplots(1, 3, figsize=(15, 5))
colors = ['steelblue', 'darkorange', 'green']
for i, (key, color) in enumerate(zip(['Density', 'Tc', 'Tg'], colors)):
    t = results[key]['true']
    p = results[key]['pred']
    r2 = r2_score(t, p)
    axes[i].scatter(t, p, alpha=0.5, color=color, s=20)
    axes[i].plot([min(t), max(t)], [min(t), max(t)], 'r--')
    axes[i].set_title(f'{key} (R²={r2:.3f})')
    axes[i].set_xlabel('Actual')
    axes[i].set_ylabel('Predicted')
plt.tight_layout()
plt.savefig('paper_figures/multitask_tg.png', dpi=300)
print("Saved: paper_figures/multitask_tg.png")
