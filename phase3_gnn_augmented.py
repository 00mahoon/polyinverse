...
for epoch in range(300):
    ...
    if (epoch + 1) % 50 == 0:
        ...
        if r2 > best_r2:
            best_r2 = r2
            torch.save(model.state_dict(), 'best_augmented.pt')

model.load_state_dict(torch.load('best_augmented.pt'))
model.eval()
y_true, y_pred = [], []
with torch.no_grad():
    for batch in test_loader:
...
print(f"\n=== Augmented GNN Best Performance ===")
...
plt.title(f'Augmented GNN (R²={r2:.3f})')
plt.tight_layout()
plt.savefig('augmented_gnn.png')
print("Saved: augmented_gnn.png")
