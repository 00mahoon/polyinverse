...
def predict(smiles):
    ...
    with torch.no_grad():
        pred_d, pred_t, pred_tg = model(graph)
...
---
*Model: Multi-task GNN (R² Density=0.793, Tc=0.536, Tg=0.321)*
"""
    return result, img
...
with gr.Blocks(title="Polyinverse v3") as demo:
    gr.Markdown("""
# 🧪 Polyinverse
## AI-powered Polymer Property Predictor & Inverse Design
**New:** Now predicts Density, Tc, and **Tg (glass transition temperature)**!
    """)

    with gr.Tabs():
        with gr.Tab("🔬 Forward Prediction"):
            gr.Markdown("### Predict Density, Tc, and Tg from SMILES")
            with gr.Row():
                with gr.Column():
