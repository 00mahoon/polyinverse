...
print(f"Density mean: {y_mean:.3f}, std: {y_std:.3f}")

graphs = []
for _, row in df.iterrows():
    normalized_target = (row['Density'] - y_mean) / y_std
    g = mol_to_graph(row['SMILES'], normalized_target)
    if g:
        graphs.append(g)
...
for epoch in range(300):
    ...
    if (epoch + 1) % 50 == 0:
        ...
        if r2 > best_r2:
            best_r2 = r2
            torch.save(model.state_dict(), 'best_gnn_v3.pt')

model.load_state_dict(torch.load('best_gnn_v3.pt'))
model.eval()
y_true, y_pred = [], []
with torch.no_grad():
    for batch in test_loader:
...
plt.title(f'GNN v3 Density (R²={r2:.3f})')
plt.tight_layout()
plt.savefig('density_gnn_v3.png')
print("Saved: density_gnn_v3.png")
