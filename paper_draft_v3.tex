...
Challenge, Extended Polymer Dataset, and TG-Density Excerpt) into a unified corpus of 4,594
unique polymer structures, our multi-task GNN simultaneously predicts three key properties:
density ($R^2 = 0.803$, MAE $= 0.0336$ g/ml), normalized crystallization temperature Tc
($R^2 = 0.681$, MAE $= 0.0262$ in normalized scale), and glass transition temperature Tg
($R^2 = 0.373$, MAE $= 68.43$ °C) from SMILES representations.
Multi-task learning improves density prediction by 35\% over single-task GNN baselines
($R^2$: $0.595 \rightarrow 0.803$), supported by a statistically significant negative
...
\begin{enumerate}
    ...
    leveraging their physical correlations across a combined dataset of 4,594 polymers.
...
\begin{itemize}
    \item Multi-task GNN achieving $R^2=0.803$ for polymer density by combining
    three open, publicly available datasets (4,594 unique polymers)
...
4,594 unique polymer structures. Table~\ref{tab:dataset} summarizes the data distribution.
...
(Left) Density ($R^2=0.803$, MAE$=0.0336$ g/ml). 
(Center) Tc ($R^2=0.681$, MAE$=0.0262$ normalized).
(Right) Tg ($R^2=0.373$, MAE$=68.43$ °C).}
...
\textbf{Multi-task GNN (ours)} & \textbf{0.803} & \textbf{0.0336} & \textbf{0.681} & \textbf{0.0262} & \textbf{0.373} \\
\bottomrule
\end{tabular}
\end{table}

Multi-task learning with the combined dataset provides a 35\% improvement
in $R^2$ over single-task GNN ($0.595 \rightarrow 0.803$).
Notably, expanding the dataset from 1,526 to 2,622 density samples further
improves performance ($0.757 \rightarrow 0.803$), confirming that data
quantity is a primary bottleneck in polymer property prediction.
The Tc $R^2$ improves substantially from 0.512 to 0.681 with the larger dataset,
while Tg prediction ($R^2=0.373$) remains limited by the smaller sample size (368 samples).
...
Our multi-task GNN achieves $R^2 = 0.803$ for density prediction by combining
three open datasets (4,594 unique polymers), outperforming single-task baselines
by 35\% and demonstrating that data quantity is a primary bottleneck.
The model simultaneously predicts Tc ($R^2=0.681$) and Tg ($R^2=0.373$),
...
Tg prediction remains limited by training data size (368 samples).
